...
AutoSequelize.prototype.build = function(callback) {
  ...
  function mapTable(table, _callback){
    self.queryInterface.describeTable(table, self.sequelize.options).then(function(fields) {
      self.tables[table] = fields
      _callback();
    }, _callback);
  }

  this.queryInterface.showAllTables(self.sequelize.options).then(function (__tables) {
        ...
        _.each(fieldAttr, function(attr, x){
          ...
          if (attr === "foreignKey") {
            ...
            if(defaultVal === null) {
              ...
              if (_.isString(val_text) && val_text.charAt(0) === '"') {
                val_text = `"${val_text.slice(1, -1).replace(/\"/g, '\\"')}"`;
              }
